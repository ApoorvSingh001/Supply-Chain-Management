...
event PartiallyPurchasedByConsumer(uint upc, uint sku)
...
//Item State Modifiers
  ...
  // modifier checkProduct(string memory _prodType){
  //   require(if(keccak256(abi.encodePacked(_prodType))==keccak256(abi.encodePacked(Egg))){
  //       ProductIdentity constant bydefault= ProductIdentity.Egg;
  //     }
  //     else if(keccak256(abi.encodePacked(_prodType))==keccak256(abi.encodePacked(Milk))){
  //       ProductIdentity constant bydefault= ProductIdentity.Milk;
  //     }
  //     else if(keccak256(abi.encodePacked(_prodType))==keccak256(abi.encodePacked(Vegetables))){
  //       ProductIdentity constant bydefault= ProductIdentity.Vegetables;
  //     }
  //   ,'NOT AVAILABLE')
  //   _;
  // }
// constructor setup owner sku upc
  ...
  function produceItemByProducer(uint _upc,uint expiryDate,uint skUnit, ProductIdentity prodtype, string memory _originFarmName, string memory _originFarmInformation, string memory _originFarmLatitude, string memory _originFarmLongitude, string memory _productNotes, uint _price) public
    ...
    // Increment sku
    //sku = sku + 1;
    UnitItems[uint8(prodtype)]=UnitItems[uint8(prodtype)]+skUnit;
  ...
  */
  function purchaseItemByConsumer(uint _upc,uint numOfUnits) public payable
    ...
    checkValue(_upc, msg.sender)
    {
      ...
      if(items[_upc].sku==0){
        ...
        itemsHistory[_upc].RTC = block.number;
        emit PartiallyPurchasedByConsumer(_upc,sku);
  ...
  function fetchItemBufferOne(uint _upc) public view returns
    ...
    (
      ...
      item.originProducerName,            //4
      item.originProducerInformation,     //5
      item.originFarmLatitude,            //6
      item.originFarmLongitude,           //7
      item.productDate                    //8
      //item.productSliced
    );
  }
